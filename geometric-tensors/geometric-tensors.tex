...
\frame{
  ...
  \begin{itemize}
    \item ``The strategy [...] is differential-geometric and information-theoretic in nature''
    ...
    \item ``besides its conceptual appeal, can have some practical relevance''
  \end{itemize}
}
...
\frame{
  ...
  \begin{itemize}
    ...
    \item Extend to a natural, complex metric: $$\gamma(\psi,\phi) = \arccos\left|\Braket{\psi|\phi}\right|$$
...
\frame{
  ...
  \begin{itemize}
    ...
    \item At each point of $\mathcal{M}$, there is a Hamiltonian $$H(\lambda)=\sum_{n=0}^{\mathrm{dim} \mathcal{H}} E_n \Ket{\psi_n}\Bra{\psi_n},\textrm{ with }E_{n+1} \ge E_n$$
...
\frame{
  ...
  ``The main message of this paper is that apparently unrelated results can be understood in unified fashion by unveiling the underlying common differential-geometric structure and analysing it quantum-critical behaviour''
}
